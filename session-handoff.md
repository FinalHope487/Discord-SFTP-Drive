...
> 455 項測試全過，突變 25/25（結構層 10 + web 層 15），pyflakes 乾淨，
> image 內同樣 455 過，**實地驗收 20/20（帳號遷移）＋ 25/25（HTTP API）。**
...
線上 `nodes` 只剩 root，Discord 上 0 個附件，`users` 一列，
`keystore` 一筆在 `user:a00e0bf3-…` 底下。

`git` 上四個 commit，`977b7d8`（HTTP API）**尚未 push**，前三個已 push。
...
2. **突變 25/25**：結構層 10 條（模型 A、共用 root…）、web 層 15 條（拿掉 CSRF 檢查、
   拿掉 session 檢查、HttpOnly、SameSite、期限夾制、絕對上限、位址層鎖定、並發上限…）。
   腳本在 scratchpad，未留在 repo。
3. **pyflakes 乾淨。**
4. **production image 內同一份 suite**：455 passed（3.12.13 / Linux）。
5. **實地驗收**：帳號遷移 20/20、HTTP API 25/25（含 12 MiB 跨兩 chunk 的
   上傳／下載／改名／刪除、cookie 三個旗標、期限夾制、CSRF 拒絕、登出後失效）。
...
1. **`977b7d8` 還沒 push。**
2. **第 5 步：前端**（純靜態 SPA）。API 已經齊了，端點列在 `ROADMAP.md`。
3. **`SFTP_PASSWORD` 走 docker secret**：已拍板要做，排在 Client UI 之後。
...
- **HTTP API 沒有引入任何新的資料格式**：它走的是同一個 `DiscordVFS`，
  寫進去的節點與 SFTP 寫的完全一樣。
...
- **相依套件**：一個都沒加。`aiohttp` 本來就在（Discord client 用），
  測試用 `aiohttp.test_utils` 而不是 `pytest-aiohttp`，就是為了不新增相依。
- **`WEB_HOST` 不是安全邊界**：容器內必須綁 `0.0.0.0`，
  `docker-compose.yml` 的 `127.0.0.1:8080:8080` 才是。改動時不要搞反。
...
1. **push `977b7d8`。**
2. **做第 5 步的前端。** 需要決定的只剩畫面與互動，API 契約已固定；
   前端是純靜態資源，改版不必重建 image。
3. 前端落地後再回頭處理 `SFTP_PASSWORD` 的 docker secret。
...
- venv 在 `venv/`（3.12.7）；上線 image 也是 3.12（3.12.13）。一律用
  `./venv/Scripts/python.exe -m pytest`（裸 `python` 是 Store 的 stub，exit 49 且無輸出）。
...
- **Windows 上用 `pathlib.write_text()` 還原檔案，會把整份檔案的 LF 換成 CRLF。**
  突變腳本就是這樣「還原」的，跑完之後三個檔案每一行都變了，而測試全過。
  要用 `read_bytes()` / `write_bytes()`，並在結尾比對 hash。
- **Git Bash 裡 `grep -c $'\r'` 不會展開成 CR**，它變成空字串、匹配每一行，
  回報的數字剛好等於總行數，看起來像「每一行都是 CRLF」。查換行符用 Python 讀 bytes。
- **突變測試要給 pytest 子行程設 timeout。** 拿掉登入佇列上限那條讓測試**卡死**而不是失敗，
  整個腳本掛在那裡，而被突變的檔案當時還沒 commit——git 救不回來。
  處理方式是只殺子行程 pytest、讓父行程的 `finally` 正常還原。
  該測試已改成用 `asyncio.wait_for` 包住，現在會快速失敗而不是 hang。
