...
class DiscordFile:
    ...
    fetched and decrypted on its own. Writes are append-only — SFTP hands us an
    offset per call, and we accept it only if it continues where the last write
    stopped.
    """

    def __init__(self, vfs: "DiscordVFS", node: dict, *, readable: bool,
                 writable: bool, write_offset: int = 0):
        self._vfs = vfs
        self._node = node
    ...
    async def write_at(self, offset: int, data: bytes) -> int:
        ...
        async with self._lock:
            ...
            if offset != self._write_offset:
                raise Unsupported(
                    "random-access writes are not supported "
                    f"(expected offset {self._write_offset}, got {offset})"
                )
...
class DiscordVFS:
    ...
    async def open(self, path: str, *, read: bool, write: bool, create: bool = False,
        ...
        if not node:
            ...
            elif not append and node["size"] > 0:
                # Rewriting in place would need random-access writes; every
                # mainstream client sends O_TRUNC for uploads, so refusing here
                # is clearer than silently corrupting the file.
                raise Unsupported(
                    "in-place overwrite is not supported; reopen with O_TRUNC"
                )

        write_offset = node["size"] if (write and append) else 0
