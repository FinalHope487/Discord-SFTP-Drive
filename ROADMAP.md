...
由當前工作長出來的衍生目標當場做掉，不寫進來——見 `CLAUDE.md`「長時工作模式／衍生目標」。
...
- [done] ~~**asyncssh 卡在 2.20.0，因為 2.21 的稀疏檔案擴充打不到 `DiscordFile`**~~
  （2026-08-11 開出，同日拍板 (b) 並落地，見下方變更紀錄與拍板決策）。
...
- [later] **多使用者的第 4 步：真正開放第二個帳號**——管理 CLI、建帳號流程、per-user 配額。
  **2026-08-10 補**：專案定位拍板為「開源自架」之後，這一步**不再是上線阻塞**——
  每個人跑自己那一份，單一帳號就是完成品。它只在日後要營運公開服務時才復活。
...
- **稀疏檔案查詢一律回答「整個檔案都是資料」，不揭露尾端的洞**（2026-08-11）。
  `DiscordFile.seek` 只服務 asyncssh 的 `ranges@asyncssh.com` 探詢，`SEEK_DATA` 回原位、
  `SEEK_HOLE` 回檔案大小，位置到了或超過結尾回 `ENXIO`。
  **不選「誠實回答」的理由是失敗方向不對稱**：`_covered_end()` 明明知道洞從哪開始，
  照實回答還能省掉傳輸那串零；但用戶端預設 `sparse=True`，而在那個模式下
  **「傳得比較少」是預期而不是錯誤**——洞的位置早報一個位元組，結果就是下載回來的檔案
  大小正確、內容是零，兩端都不會有任何一行 log。宣稱全是資料則不可能這樣壞，
  而且那正是 2.21 之前的協定行為，所以沒有任何傳輸改變形狀。
  **省下來的頻寬只有稀疏尾端那一段**，等到真的量到它在浪費頻寬再回來換，不是現在。
  **`seek` 刻意不是通用的**：這個 handle 沒有檔案位置（讀取自帶 offset），
  所以其他 `whence` 一律 `NotImplementedError`，而不是回一個呼叫端有資格相信的數字。
  代價與提醒：這條規則只有 Linux 會執行到，**Windows 上測不出任何差別**，
  所以 `tests/test_sparse_ranges.py` 直接對 `seek` 斷言，不繞路走真的用戶端。

- **外殼的語言偏好由主程序擁有，存在 `config.json`**（2026-08-11）。不放在頁面裡，
  因為 `setup.html` 與 `local.html` 都是 `file://` 而且互相連結：**頁面持有的值在第一次
  點擊那個連結時就沒了**，重開 app 也沒了。頁面透過 `dd:language` / `dd:setLanguage`
  讀寫，值一律過 `normaliseLanguage()`——`config.json` 是使用者可以手動編輯的檔案，
  而且舊版本寫出來的那份根本沒有 `lang` 這個鍵，所以「不是語言」是正常輸入不是錯誤。

- **兩頁各自 inline 一份字典，不共用檔案**（2026-08-11）。**這是 CSP 決定的，不是偷懶**：
  這個視窗的 policy 是 `script-src 'unsafe-inline'`，沒有 `'self'`，所以
  `<script src="i18n.js">` 會被擋。要共用就得放寬那道 policy，而那個視窗掛著特權 bridge。
  代價其實不存在——兩頁沒有任何一句話是共用的，重複的只有二十行查表機制。
  **翻譯字串裡只允許 `` `code` `` 與 `*bold*`，而且用 `createElement` 建成真元素，
  不走 `innerHTML`**：翻譯是資料，資料不在特權頁面上被當成標記解析。

- **可見文字之外，控制項的無障礙名稱要包含那段可見文字**（2026-08-11）。語言按鈕的
  可見標籤是「你現在在哪個語言」（中文／EN），它沒有說這個控制項是做什麼的；
  但把名稱換成「語言」又會讓語音操作的人念出可見標籤卻叫不動它。
  結論是 `語言：中文` / `Language: EN`——兩者都說到，且可見標籤在名稱裡面。

- **外殼與檔案管理員的語言偏好維持獨立，不接起來**（2026-08-11，`QUESTIONS.md` 第 1 題，
  選 A）。外殼兩頁的偏好在 `config.json`，檔案管理員的在 `localStorage`（`dd.lang`，
  作用域是伺服器 origin）；SPA 由遠端伺服器提供，外殼不去碰它的 storage。
  **代價是接受的**：在 setup 選了英文，開啟硬碟視窗之後 SPA 仍是它自己上次記得的語言，
  要再切一次；换成外殼帶 `?lang=` 覆蓋的話，使用者在 SPA 裡切過的語言會被靜默蓋掉，
  而他不會知道是誰蓋的——那個代價比「切兩次」更貴。

- **外殼預設語言固定 `zh`，不跟系統語系走**（2026-08-11，`QUESTIONS.md` 第 2 題，選 A）。
  `config.json` 沒有 `lang` 鍵時如此，`app.getLocale()` 不使用。
  **理由是既有測試**：`tests/test_ui_shell.py` 有 11 條斷言中文字串，改成跟系統語系走
  會讓斷言隨 CI runner 的語系而變，要全部先寫死 `lang` 再斷言——值得做但是獨立一輪的事，
  不是這一輪「決定要不要」的範圍。

- **`GUIDE.md` 不進「Where things are」表格，`local.html` 的中文首次執行畫面改指
  `README.md`**（2026-08-11，`QUESTIONS.md` 第 3 題）。**選項 A 的前提是錯的，
...
- **專案定位是「開源自架」，不是「我營運一個公開服務」**（2026-08-10）。
  ...
  **因此現有的單一帳號模型就是完成品，不是半成品**——多使用者第 4 步
  （管理 CLI／建帳號流程／per-user 配額）維持 `[later]`，且**不預先承諾**；
  「只能跑一個副本」在自架情境下不再是限制。日後要開公開服務是獨立決定，
  屆時第 4 步、密碼救援路徑、配額與濫用處理、隱私政策全部才會變成阻塞。

- **授權條款是 Apache-2.0**（2026-08-10）。選它不選 MIT 的理由是那段明確的免責聲明：
  這是加密儲存工具，**使用者弄丟密碼就是永久失去資料**，「不負擔保責任」要是白紙黑字
  而不是靠 README 的一行提醒；附帶的專利授權也比 MIT 完整。否決 AGPL——
  這個專案希望被拿去用，不是希望管住拿去用的人。
  **不逐檔加授權標頭**：Apache-2.0 不要求，`LICENSE` 檔就夠，84 個檔案各加 15 行
  只是讓每個 diff 多一段噪音。**這一項不可逆**——已依此版取得程式碼的人永遠保有這個授權。

- **Discord ToS 的灰區風險明確揭露，不埋起來**（2026-08-10）。查證結論分三層：
  一般 ToS **沒有**明文禁止拿它存檔案；但 Developer ToS 要求 API 使用符合文件用途、
  不得超出速率限制，並保留在認為某開發者「對平台造成負面影響」時終止其存取的裁量權；
  沒有找到針對這類專案大規模執法的公開紀錄。**所以它是灰區風險，不是已知違規。**
  真正改變的是風險落在誰身上——開源之後是**每個自架者拿自己的帳號承擔**，
...
- **`README.md` 是唯一的公開文件，`GUIDE.md` 與 `BUILD.md` 轉為內部**（2026-08-10，
  **2026-08-11 修訂為「每個事實只有一個家」，見本節第一條**）。
  ...
  `README.md:108` 寫錯測試數這件事已經發生過兩次。
  `ROADMAP.md` / `SOP.md` / `CLAUDE.md` / `design-*.md` 維持公開且維持中文：
  它們是「為什麼長這樣」的權威來源，`README.md` 指過去並標明語言。
  `session-handoff.md` 與 `QUESTIONS.md` 移出版控——收件人是下一輪的自己，對外沒有意義。
...
**2026-08-20 · 委派寫測試的收貨規則（`CLAUDE.md`）與一條量測技術（`SOP.md`）** — 測試數
不變（774 過 18 skip），程式與測試零改動。

- 起因是一組在 `%LOCALAPPDATA%` 副本裡跑的委派實驗；實驗記錄本身不進版控，這裡只留
  轉成規則的兩條結論。
- **委派寫測試要先給 `git diff` 的函式清單。** 不給軸的三個回合交出 212 支測試、4557 行、
  全綠，對三個既有缺陷殺傷率 **0**；同一個模型給了函式清單後，19 支測試全中，還逐一標出
  `檔案:行號`。
- **收貨不看綠不綠。** 那 212 支測試用 `trace --count` 量，**執行到了**三行缺陷
  （8～11 次、1～5 次、1～2 次）然後什麼都沒斷言——「沒覆蓋到」與「覆蓋到了沒斷言」
  在全綠輸出裡長得一模一樣。區分方法（標準庫 `trace`，不必新增相依）進了 `SOP.md`。
- 順帶記一個沒被推翻的盲區：不指定驅動層時，四個回合 231 支測試走真實協定堆疊的是 **0** 支。
...
- **拍板 (b)，理由見上方拍板決策。** 拿回 2.21.0 的金鑰交換強化、2.23.0 的 server config
  使用者代換封鎖、2.23.1 的 SCP 路徑穿越修補。
- **這一輪最該記住的不是那四行**，是**「本機全綠」對跨平台分支毫無意見**。
  `os.SEEK_DATA` 在 Windows 不存在，asyncssh 因此在匯入時就綁了另一份不 seek 的
  `_request_ranges`——所以 `test_put_get.py` 在這台機器上不管方法對、錯、還是根本不存在
  都是綠的，而它在 Linux CI 上是紅的。**新測試因此直接對 `seek` 斷言，不繞路走真的用戶端**：
  繞路的那條在唯一能執行它的平台以外沒有牙齒。
- **`ENXIO` 那條不是防禦性程式碼**，是終止條件。`limit` 來自用戶端自己更早的 `fstat`，
  所以它可能超過檔案現在的長度；POSIX 在結尾及之後回 `ENXIO`，而 asyncssh 正好只吞這個
  errno，於是迴圈乾淨結束而不是一直產出零長度區間。
...
**2026-08-11 · `QUESTIONS.md` 三題語言/文件問題結案** — 787 過、`node --test` 23 過
（沒動測試數，第 1、2 題是「維持現況」、第 3 題是文件與一句產品文案）。

- 第 1、2 題（語言偏好獨立、預設語言固定 `zh`）都是選 A＝現況，零程式改動，
  只把決策搬進上方拍板決策。
- **第 3 題動手前發現前提是錯的**：`QUESTIONS.md` 寫的選項 A 假設 `GUIDE.md` 能留著當
  「中文入口」被 `README.md` 連到，但 `GUIDE.md` 在 `.gitignore:42`，跟 `BUILD.md` 同類，
  `git clone` 拿不到，`client/shell/package.json` 的 `files` 清單也沒帶它進打包後的 app。
  **`client/shell/local.html` 的中文首次執行畫面在這之前就已經指著一個使用者機器上
  永遠不存在的檔案**——不是這一輪引入的，是查前提時才發現的既有 bug。
  照原計畫做（選項 A）會把這個不存在的檔案寫進 `README.md` 公開的表格裡，
  等於把已知會壞的引用再放大一份。**改落地選項 C**：`local.html` 中英文兩版現在都指向
  `README.md` 的 Step 1，`GUIDE.md`（未進版控）縮成一份本機參考，不再被任何追蹤檔案引用。
  動了兩個追蹤檔案：`client/shell/local.html`、`README.md`（撤掉原本計畫加的那一列）。
  **沒有測試斷言舊字串**，改動前先 grep 過 `tests/*.py` 與 `client/shell/*.test.js` 確認。

**2026-08-11 · `CLAUDE.md` 的內部矛盾清掉，驗證數字回填** — 787 過、`node --test` 23 過
（沒動程式，這兩個數字是回填，不是新增）。

- **12 處規則互相打架，4 處由你拍板**：中風險層從「經確認才動工」改成「留痕不等確認」、
  PR 只在後續工作依賴它合併時才停手、衍生目標當場做不進這份文件排隊、
  「不確定一律視為高風險」收窄成只對不可逆／影響 `main`／production／既有資料的動作。
  其餘 8 處是詞義與互鎖：「輪」同時指一次委派和一次重試、「修好它是唯一的事」與「修 3 輪就換下一件」
  互相卡死、加密／認證同時落在兩個處置不同的層。
- **文件寫的數字會自己過期，而且沒有任何東西會變紅**：`CLAUDE.md` 停在 775／16，
  同一個分支的這份變更紀錄早就是 787／23。差距是這一輪自己加的 12 項——
  **寫數字的人和讀數字的人是同一個，所以沒人對帳**。
- **委派 subagent 那 16 行退役到 `SOP.md`**：委派本身已列進高風險層，未經要求不執行，
  規格就每輪被讀、每輪用不到。`CLAUDE.md` 只留高風險清單裡的一行。
  同一節裡「結論必須附 `檔案:行號`」管的是我自己的結論，與 subagent 無關，搬進〈驗證〉留下。
- **回填時才發現第三個前置條件從沒被寫下來**：`client/app/dist/` 在 `.gitignore:52`，
  `git clone` 之後不存在，`built_client` 對它 `pytest.fail`，於是 10 項直接紅。
  原本那張表只列會 skip 的三項——**會 fail 的前置條件比會 skip 的更該寫，卻因為它不沉默而被漏掉**。

**2026-08-11 · 外殼兩頁有了英文與語言開關，SPA 的語言開關第一次被證明會動** —
787 項（+12：`test_ui_shell.py` +7、新檔 `test_ui_language.py` +5）、`node --test` 23 過（+7）。
**12/12 突變全被抓到**（分兩檔四輪：外殼四個突變殺掉 7 條，SPA 兩輪殺掉 5 條）。

- **這一輪之前，`client/app` 的英文字典與那顆切換晶片沒有任何測試**。字典從專案早期就在，
  chip 也在，764 項全綠——**而 `document.documentElement.lang` 從頭到尾是寫死的 `zh-Hant`**。
  英文介面被螢幕閱讀器當中文念、被瀏覽器當中文斷字，沒有任何一條測試會變紅。
  教訓與「假件不模擬的那一半」同型：**功能存在不等於功能被證明過**，
  而「介面看起來是英文的」正好是目測會通過、機器沒在看的那一類。
- **最有價值的斷言是「切成英文之後畫面上不該剩任何中文字」**，不是逐句比對。
  逐句只涵蓋想得到的那幾句；掃描涵蓋想不到的。實證：`Login.jsx` 的 `labelFor()`
  用數字加單位組出「10 分鐘」，它不經過字典，**任何逐句斷言都碰不到它**。
- **外殼的字典為什麼不能共用一份**：CSP 只給 `script-src 'unsafe-inline'`。
  第一次寫的時候直覺是抽共用檔，抽完才會發現被擋——**先看 CSP 再決定檔案怎麼切**。
- **狀態訊息要跟著語言重繪**：probe 的失敗訊息是在按下按鈕那一刻組出來的，
  只翻譯 `[data-i18n]` 元素會留下一段中文錯誤壓在英文頁面上。做法是把狀態存成
  key + 變數而不是存成算好的字串，`applyLanguage()` 再呼叫一次 `renderStatus()`。
  這與 `client/app/src/i18n.js` 開頭記的那個 bug 是同一個形狀的問題。
- **`local.html` 原本把第一次執行的使用者指去 `GUIDE.md`，而那份只有中文**。
  英文字串已改指 `README.md` 的 Step 1；`GUIDE.md` 本身怎麼處理見 `QUESTIONS.md` 第 3 題。
- **觀察到一次 `test_a_rejected_token_puts_the_backend_output_on_the_screen` 偶發紅**
  （這一輪沒碰它）：`start()` 時 `LocalDrive` 的 child 已經不在，畫面上是
  「the backend is not running」。同一份程式碼之後連跑三次 18/18 全綠，單獨跑也綠。
  沒有寫進 `SOP.md`——只出現一次，且不確定是否環境決定。**再出現一次就寫。**
- **這一輪刻意不做的相鄰修正**：`client/README.md` 的外殼檔案清單（本來就過期，
  ROADMAP 已有 `[later]` 記著，這輪又多了 `language.js` / `language.test.js` 沒補進去）、
  `src/web.py` 那張「前端還沒建置」中文優先的 fallback 頁（只有建置失敗才看得到）、
  以及 `requirements*.txt` 裡別輪留下的未提交相依調整（高風險項，沒碰）。
...
- **四項拍板見上方，不複述。** 這一輪的形狀是「把一個私人專案變成別人能拿去用的東西」，
  而阻塞它的不是功能，是四件與程式碼無關的事：沒有授權條款、風險沒揭露、
  文件裡有個人識別資訊、沒有人能驗證 clone 下來跑不跑得起來。
- **沒有 `LICENSE` 的公開原始碼在法律上是「保留所有權利」**——看得到但不能合法使用、
  修改或散布。「開源」這兩個字在補上 `LICENSE` 之前是不成立的，這是唯一的硬阻塞。
- **git 歷史是乾淨的**：`.env`／`secrets/`／`host_key`／`*.sqlite3` 從未被提交過（82 個 commit 全查）。
  但**追蹤中的文件裡有兩處個人識別資訊**——`ROADMAP.md` 的真實 email 與 `SOP.md` 的
  本機路徑與使用者名稱。**兩者都是進了公開歷史就洗不掉的東西**（要改寫歷史加 force push），
  所以在轉 public 之前處理，不是之後。
- **`pyflakes` 在寫 CI 的時候當場抓到一個 `tests/test_ui_login.py` 沒用到的 import。**
  值得記的是它為什麼沒被發現：前幾輪的變更紀錄都寫著「pyflakes 乾淨」，
  但那是**當時跑過**的結論，而 2026-08-09 新增的測試檔之後沒有人再跑一次。
  **把 lint 寫進 CI 與「這一輪跑過 lint」是兩件事**，前者才會擋住下一個人。
- **CI 的三個指令都不需要憑證也不需要網路**，所以 workflow 不必碰任何 secret。
  兩個外部前提要先備好：`client/app/dist` 必須先建（`built_client` fixture 對過期的 bundle
  是 **fail 不是 skip**），以及 `playwright install chromium`。
  `node --test` 那半在 CI 上會少跑 5 項——驅動真 `discord-drive.exe` 的那組
  **自己 skip 掉**（`backend.test.js:48` 檢查檔案存在），因為 PyInstaller 只能打自己平台的包。
- **CI 本身尚未在 GitHub 上跑過一次**，那要等 repo 轉 public（或手動觸發）才驗得到。
  在那之前 `.github/workflows/test.yml` 的綠燈是**預期值，不是量到的**。

**2026-08-09 · 決策不再被壓縮吃掉，文件數字不再自己漂走** — 已提交樹 752 項（+27），
工作目錄 764 項；`--db=sqlite` 753 過 3 skip；`node --test` 已提交樹 6 過、工作目錄 16 過。
新增的 27 項是 `test_decision_log.py`。

- **`PreCompact` hook 把決策抽進 `DECISIONS.jsonl`。** 補的是「留痕全部發生在一輪結束前，
  但壓縮發生在中間」這個時間差。
- **單元測試全綠不代表對真輸入有用**，這是 `SOP.md` 第 63 條的第二次：27 項綠的版本
  對 33 MB 真 transcript 抽出 114 條，其中三分之二是「需要你決定」——待決問題，決策的反面。
  過濾第二人稱與清單引導句之後降到 68 條、精確度約三分之二。真 transcript 的假陽性已變成測試案例。
- **文件裡的驗證數字漂了，而且兩個方向都漂。** `CLAUDE.md` 寫 714 / 16，已提交樹實際是 725 / 6。
