...
- [later] **(BP) 兩處小整潔問題**，不修也不會壞：
  - `_url_cache`（`src/discord_api.py:97`）沒有上限，只有 `delete_message` 會移除。
    每筆約 200 bytes，十萬個 chunk 才 20MB，實務上不會出事。真要修就換有上限的 LRU——
    **注意這個快取淘汰是安全的**（查不到只是多打一次 API），與 `_node_versions` 不同。
  - `_to_sftp_error`（`src/sftp.py:59-62`）的 `elif (AlreadyExists, NotEmpty, NotADirectory,
    IsADirectory)` 與 `else` 都回 `FX_FAILURE`，沒有行為差異，但讀起來像是那四種有專屬狀態碼。
    刪掉該分支或加一行註解說明「v3 沒有更精確的碼，此處為 v4/v6 預留」。
...
- [later] **失敗過的 handle 在「讀」這一側沒有一致的語意**（2026-08-01 補失敗路徑測試時發現）。
  `_failed` 只擋寫入路徑（`write_at` / `truncate_to` / `close`），`read_at` 不看它，
  所以失敗後：(1) `size` 仍然把還在 buffer 裡、實際上永遠不會落地的位元組算進去；
  (2) `read_at` 仍然會嘗試 `_flush_buffer()`——如果 Discord 這時剛好復原了，
  那次上傳會成功並 commit，等於在一個已經拒絕寫入的 handle 上復活了資料。
  **不是資料遺失**（那些位元組確實是客戶端寫的），而且客戶端已經收到寫入失敗、
  正常情況下不會再用這個 handle。但語意不一致，值得收乾淨。
  修法大概是讓 `read_at` 在 `_failed` 時跳過 flush、`size` 不計入 buffer——
  要一起想清楚才動，所以不是順手改。
