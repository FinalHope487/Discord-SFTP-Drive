...
- [next] **同一類 unwind 缺口要在 SFTP 那一側也查一次**（2026-08-06 開出）。
  這一輪修掉的兩條都在 HTTP 路徑上。SFTP 的 handle 由 asyncssh 管生命週期，
  連線被砍時走的是另一條路，**沒有驗過它會不會用同樣的方式 commit 半個檔案**。
  `_rollback()` 的修正是共用的（在 `DiscordFile` 上），所以目錄鎖死那條 SFTP 也一起好了；
  沒把握的是「斷線會不會被當成正常結束」。驗法照 `tests/test_upload_disconnect.py`：
  真的砍連線，然後看節點還在不在。

- [next] **只有真依賴才驗得了的東西，還剩兩項半**（2026-08-06 從 `session-handoff.md` 移入
  ——那個檔案每輪重寫，繼續留在那裡等於下一輪就消失；同日驗掉其中兩項半）。共同點是
  `tests/fakes.py` 自己宣告了不模擬這些，所以**綠色的測試對它們不構成證據**；`SOP.md`
  那條推論已經用垃圾桶索引示範過一次代價（測試綠了三天，服務起不來）。
  - ~~**`_rollback()` 的保護在 HTTP 這個新呼叫點上真的有效**~~ **已驗**（2026-08-06）。
    上傳 30 MiB 途中停掉 MongoDB，逐項驗過附件真的被收回。**同時抓到一個真 bug**，見下方決策。
  - ~~**session 真的閒置到期**~~ **已驗**（2026-08-06）。不是把時鐘往前撥，是真的閒置超過
    上限；UI 退回登入畫面且沒有靜默重試。
  - **用真密碼在 UI 上手動點一輪** — **剩一半**。對真依賴的那一半已驗（2026-08-06，
    dev stack：真 MongoDB、真 Discord、跨分塊上傳下載 sha256 相符），腳本可重跑。
    **沒驗的是**：真人用滑鼠鍵盤走一次（自動化走的是元素自己的 handler，不經過 OS 輸入層），
    以及 production 自己的資料與金鑰。真密碼是主金鑰的包裝，刻意不讓它進入對話或指令紀錄，
    **所以剩下這半只能由人自己點**，不是可以委派的工作。
  - ~~**附件 URL 真的過期之後的重取路徑**~~ **邏輯已驗，時鐘那半沒有**（2026-08-06）。
    `tests/test_url_expiry.py` 換掉了「假件的 URL 永遠有效」這個前提：stub 會簽出帶
    `ex=` 的 URL 並**真的在過期後回 404**，所以預測性重取（讀 `ex`、留安全邊界）與
    反應性重取（403/404 後再解析一次）兩條分支都跑得到，14 項通過。
    做過突變測試：拿掉安全邊界或拿掉 403/404 重試，各有測試變紅。
    **仍然沒驗的是**：Discord 自己的 CDN，以及真的等 24 小時讓簽章自然過期。
    stub 符合這個 client 解析的格式，但 Discord 那端改格式的話這裡不會知道。
  - ~~**上傳中途斷線的清理路徑**~~ **已驗，而且答案是「壞的」**（2026-08-06）。
    見上面的 `[now]`：真的砍連線之後才發現沒有清理路徑可言。這一項從驗證缺口
    變成了兩個待修的缺陷。
...
- [next] **重新產出 `BLUEPRINT.md`**。它以 `e288ff7` 為準，之後 H1 與 H2 都落地了，
  ...
  2026-08-06 從 `[later]` 提上來。現在它還缺的是整個 HTTP 層、前端、桌面外殼與 jobs。
  兩個前置條件都已解除：docker secret 動的 `config.py` 設定載入路徑、以及
  `_rollback()` 的行為（§4 要描述的正是它），都已於 2026-08-06 落地。
  **這是目前 `[next]` 裡唯一還沒動的一條。**
