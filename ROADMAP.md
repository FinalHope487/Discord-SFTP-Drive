...
- [next] **asyncssh 卡在 2.20.0，因為 2.21 的稀疏檔案擴充打不到 `DiscordFile`**
  （2026-08-11，CI 上量到的）。2.21.0 加了 `ranges@asyncssh.com`，**用戶端 `get()`
  預設就會問**，而伺服器端直接對 handle 物件呼叫 `lseek(SEEK_DATA)`——繞過本專案所有
  `SFTPServer` override，跟 `src/sftp.py` 開頭記的那兩個坑同一個形狀。
  **Windows 上永遠是綠的**：`os.SEEK_DATA` 在那裡不存在，asyncssh 綁的是另一份不 seek 的
  實作。所以本機全綠、Linux CI 紅，而正式部署是 Linux。
  **修的方向比 bug 危險**：`sparse=True` 之下「傳得比較少」不算錯誤，所以答錯洞的位置
  ＝下載回來是一堆零、沒有任何錯誤。兩條路各有代價，要拍板：
  - **(a) 誠實回答**：本專案的檔案真的有稀疏尾端（2026-07-31 那條決策），
    `seek(SEEK_DATA/SEEK_HOLE)` 照 chunk 佈局回答，順便省掉傳輸尾端那串零。
    代價是它是同步方法而佈局來自可能過期的 `_node`，答短了就是靜默的零。
  - **(b) 一律宣稱整個檔案都是資料**（四行，`SEEK_DATA` 回原位、`SEEK_HOLE` 回檔案大小）。
    行為與 2.21 之前完全相同，沒有靜默失敗的空間，也沒有省到任何東西。
  - **(c) 維持現狀不升級**。代價是放棄 2.21.0 的金鑰交換強化、2.23.0 的 server config
