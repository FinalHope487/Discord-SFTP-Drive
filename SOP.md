...
每條開頭註明 `(日期・工具/模型版本)`——這條是針對哪個版本的行為寫的，決定了它什麼時候該退場。
...
註明退役日期與原因，不要留在檢查清單裡佔位。退役不是刪除——留著才知道曾經踩過。
...
`python 指令 exit 49 且完全沒有輸出（pip / venv / 跑測試都一樣）→ 1. which python：若指向 AppData\Local\Microsoft\WindowsApps\python，那是 Microsoft Store 的轉址 stub，不是直譯器，永遠 exit 49 2. 改用真正的直譯器絕對路徑（本專案 venv 的來源是一份 3.12 的 Anaconda 安裝）3. venv 位於專案根目錄的 venv/（已在 .gitignore 內）；一律用 ./venv/Scripts/python.exe 跑測試與 lint，不要用裸 python → 環境 PATH 解析到假的直譯器`
...
`跑 pytest 看不到最後那行 N passed（進度點跑到 100% 就結束，exit code 正常）→ 1. 不要懷疑測試沒跑或 pytest 壞了 2. pytest.ini 的 addopts 已經是 -q，命令列再加一個 -q 就成了 -qq，靜默等級升一級、摘要行整行被吃掉 3. 要數字就不要自己補 -q；只想關警告用 -p no:warnings → 設定檔與命令列旗標疊加，非測試或 pytest 問題`
（2026-08-20・pytest 8 系列。CLAUDE.md〈驗證〉要求「看到綠不算數，要看數字」，而這個坑正好讓數字消失、其他一切看起來都正常，代價是白跑一次全套件。）
...
`要知道一套測試有沒有真的執行到某幾行，又不想為此新增相依 → 1. 不要裝 coverage/pytest-cov，那命中「新增或移除相依套件」的高風險層 2. 標準庫就夠：python -m trace --count --coverdir=<dir> --module pytest 3. 讀 <dir>/<模組路徑>.cover，每行前面的數字就是執行次數，沒執行的是空白 4. 全套件跑一次約慢 8 倍（0.9s 變 7s），可接受 → 標準庫 trace，非缺工具`
（2026-08-20・Python 3.12・pytest 9.1.1。用途是區分「測試沒覆蓋到」與「覆蓋到了但沒斷言」——這兩件事在全綠的輸出裡長得一模一樣，而後者才是委派來的測試最常見的失效模式。）

## 已退役

<!-- 條目退場時搬到這裡，註明退役日期與原因。留著才知道曾經踩過 -->

### 主 Agent 委派 Subagent 的結構化邊界（2026-08-11 退役．2026-08-18 恢復）

**已恢復，全文在 `CLAUDE.md`〈委派邊界規格〉。** 這裡只留歷史，不留第二份全文——
兩份會漂開，而漂開時沒有任何東西會變紅。

當初的退役原因：委派這件事本身被關掉了，不是那五項邊界失效。委派 subagent 列在
`CLAUDE.md` 的高風險層、未經使用者要求不執行，於是那 16 行每一輪都被讀進去、
每一輪都用不到。**委派現在仍然是高風險**，這次恢復的是「一旦獲准委派，prompt 該長什麼樣」，
不是把委派打開，所以當初的退役理由並沒有被推翻。

恢復時採用 collab-kit 現行版，不是原樣撈回原本那 16 行（原本的指示是原樣撈回，
這次是使用者指定改用現行版）。差別有二：受託方的模型選擇從寫死 Sonnet 改成
「固定的較便宜模型」並寫明成本效益前提；新增「一次開多個受託方要先報備」。

**沒有跟著退役的一句**：「任何『沒問題／可以了／檢查過了』的結論必須附 `檔案:行號` 或指令輸出」
與 subagent 無關——它管的是我自己的結論——已搬進 `CLAUDE.md`〈驗證：不接受目測／規則〉。
