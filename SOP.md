...
### 主 Agent 委派 Subagent 的結構化邊界（2026-08-11 退役．Claude Opus 5）

**退役原因：委派這件事本身被關掉了，不是這五項邊界失效。** 委派 subagent 現在列在
`CLAUDE.md` 的高風險層，未經使用者要求不執行，於是這 16 行每一輪都被讀進去、每一輪都用不到。
恢復委派時原樣撈回 `CLAUDE.md`，不要憑印象重寫。
...
Subagent 碰到需要拍板的事，一律回報給主 agent 彙整進 `QUESTIONS.md`，不自己決定也不直接問使用者。

Subagent 必須總是使用 Sonnet 模型，不論主模型種類。

Subagent 回報「完成」不構成完成。主 agent 收到報告後必須自己跑 `git diff` 確認宣稱的改動
真的存在、自己跑一次驗證指令，才可以轉述。兩者不一致時以 diff 與指令輸出為準，
並在回報裡指出報告哪裡不實。
