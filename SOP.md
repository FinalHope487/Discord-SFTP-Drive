...
（註：2026-08-01 寫入。這是同一類問題第三次出現：先是 SFTPServer 方法簽章對不上、asyncssh 從未呼叫到覆寫的 I/O 方法；再是 SIGTERM 的 flush 不在 wait_closed() 等待範圍內、log 一切正常但資料消失；這次是 scandir 繞過 list_dir，讓目錄子項集合的完整性保護對真實客戶端完全無效，而 371 項測試全綠。三次的共同點都是「VFS 那半邊是對的」。tests/test_sftp_e2e.py 這個 suite 最初存在的理由就是這件事。）
